...
with change_data as (
    ...
    select
        ...
        , sum(case when unioned.{{ col.name }} is null and not coalesce(details.{{ col.name }}, true) then 0
                ...
                partition by unioned.lead_id {{ fivetran_utils.partition_by_source_relation(package_name='marketo', alias='unioned') }}
    ...
    select 
        ...
        , last_value(field_partitions.{{ col.name }}) over (
            partition by field_partitions.lead_id {{ fivetran_utils.partition_by_source_relation(package_name='marketo', alias='field_partitions') }}
        ...
        , first_value(field_partitions.{{ col.name }}) over (
            partition by field_partitions.lead_id, field_partitions.{{ col.name }}_partition {{ fivetran_utils.partition_by_source_relation(package_name='marketo', alias='field_partitions') }}
