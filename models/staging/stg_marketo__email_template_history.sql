with base as (
    ...
    select  
        *,
        row_number() over (partition by source_relation, email_template_id order by updated_timestamp) as inferred_version,
        count(*) over (partition by source_relation, email_template_id) as total_count_of_versions
    from fields
    ...
    select 
        *, 
        ...
        lead(updated_timestamp) over (partition by source_relation, email_template_id order by updated_timestamp) as valid_to,
